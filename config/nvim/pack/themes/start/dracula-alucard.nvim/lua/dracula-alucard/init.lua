...
local DEFAULT_CONFIG = {
    ...
    colors = require("dracula-alucard.palette"),
    overrides = {},
    theme = "dracula-alucard",
}
...
local function apply(configs)
    ...
    local groups = require("dracula-alucard.groups").setup(configs)
...
local function get_configs()
    ...
    if g.colors_name == "dracula-alucard-soft" then
        configs.theme = "dracula-alucard-soft"
        configs.colors = require("dracula-alucard.palette-soft")
    elseif g.colors_name == "dracula-alucard" then
        configs.theme = "dracula-alucard"
        configs.colors = require("dracula-alucard.palette")
    end
...
local function load(theme)
    if vim.fn.has("nvim-0.7") ~= 1 then
        vim.notify("dracula-alucard.nvim: you must use neovim 0.7 or higher")
        return
    end
    ...
    g.colors_name = theme or "dracula-alucard"
